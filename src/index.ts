...
app.post("/initialize", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json({ msg: `No user with email ${email} found` });
    ...
    if (user.hasInitialized && user.paystackReferenceCode) {
      ...
      try {
        const verifyResponse = await axios.get(
          `https://api.paystack.co/transaction/verify/${user.paystackReferenceCode}`,
          {
            headers: {
...
app.post("/chargeAuthorization", async (req: Request, res: Response) => {
  const email = req.query.email as string;
  const { amount } = req.body;

  if (!email) return res.status(400).json({ msg: "Email is required" });
  if (!amount || amount <= 0) return res.status(400).json({ msg: "Amount must be a positive number" });
